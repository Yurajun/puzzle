...
$('.game-floor__button-right').on('click', function (){ // вправо
	let shiftLeftCss = parseFloat($(game.gameFloorPuzzleLine).css('left'), 10);
	const sl = document.querySelector('.game-floor__puzzle-line').getBoundingClientRect().left;
	let shiftLeftProp = parseFloat($(game.gameFloorPuzzleLine).prop('left'), 10);
	if ((sl + 5) < game.pieceSizeLine){
		$(game.gameFloorPuzzleLine).css('left', shiftLeftCss += numberShift);
		$(game.gameFloorPuzzleLine).prop('left', shiftLeftProp += numberShift);
...
$('.game-floor__button-left').on('click', function (){ // влево
	let shiftLeftCss = parseFloat($(game.gameFloorPuzzleLine).css('left'), 10);
	let shiftLeftProp = parseFloat($(game.gameFloorPuzzleLine).prop('left'), 10);
...
function toggleHelpImage(){
	if (!helpImageFlag){
		$('.game-floor__look-full-img').addClass('game-floor__look-full-img_help');
		$('.game-floor__button-full-img').addClass('game-floor__button-full-img_help');
		$('.image-help').show();
		$('.game-content__game').css({'z-index': 0});
		helpImageFlag = true;
		game.sounEffect('preview.wav', 'preview.mp3', 'preview.ogg', '0.3');
	}else {
		$('.game-floor__look-full-img').removeClass('game-floor__look-full-img_help');
		$('.game-floor__button-full-img').removeClass('game-floor__button-full-img_help');
		$('.image-help').hide();
