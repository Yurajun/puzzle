...
$('.folder-select').click(function() {
	setTimeout(function() {
		...
		$('.remodal__modal-select')
			.find('.jspDrag')
			.addClass('jsp-drag_select');
	}, 500);
});
$(function() {
	$('.scroll-pane').jScrollPane();
...
function showGame(curentImg) {
	...
	setTimeout(function() {
		...
		game.cleanFiled();
		setTimeout(function() {
			game.load(imgName, level, false, true, false);
...
function replaceLanguage() {
	...
	if (cookLenguage === '1') {
		...
		$('.resum-p').html(
			'Continue <br /> <span class="resum-p__span">build</span>'
		);
		...
		$('.remodal-games__p').html(
			'Try our other games <br /> on the Windows Store, Google Play and the App Store'
		);
		$('.remodal-options__p').html(
			'<span class="remodal-options__p_eng-style">The complexity<br /> of the puzzle: </span>'
		);
		...
		$('.remodal-about__p_desc').html(
			'Lovely Puzzle will allow you to choose a lot of puzzles from the library game of the game, <br /> and also will be able to download any picture in JPG format from your <br /> computer, turning it into a puzzle piece, which can also be collected. <br /> When you exit the game the puzzle is saved, so you can always <br /> continue to collect it later.'
		);
		$('.remodal-about__p_devel').html(
			'<p class="remodal-about__p remodal-about__p_font-size"><span class="remodal-about__p-span">In development participated:<br /></span>Ulyashchenkov Sergey <br /><span class="remodal-about__p-span">(Author of the idea, design, interface),<br /> </span>Komarov Yura <span class="remodal-about__p-span">(programming),<br /></span>Svetlana Fokina <span class="remodal-about__p-span">(second author).</span></p>'
		);
		...
		$('.resum-p').html(
			'Продолжить <br /> <span class="resum-p__span">сборку</span>'
		);
		...
		$('.remodal-games__p').html(
			'Попробуйте другие наши игры <br /> в Windows Store, Google Play и App Store'
		);
		...
		$('.remodal-about__p_desc').html(
			'Lovely Puzzle позволит вам выбирать множество паззлов из библиоткеи игры, <br /> а также даст возможность загрузить любую картинку в формате JPG с вашего <br /> компьютера, превратив ее в паззл, который также можно собрать. <br /> При выходе из игры паззл сохраняется, поэтому вы всегда сможете <br /> продолжить собирать его позже.'
		);
		$('.remodal-about__p_devel').html(
...
$('.remodal-lenguage__label-level, .remodal-lenguage__label-level2').on(
	'click',
	function() {
		$(this)
			.siblings()
