...
game.createPuzzleItem = function (){
	...
	for (let i = 1; i <= game.puzzleQuantity.rows; i++) {
		for (let j = 1; j <= game.puzzleQuantity.columns; j++) {
			...
			const svg = game.createPuzzle(game.image, ++w, game.itmeSize, row[j - 1][i - 1], col[i - 1][j - 1]);
...
game.createPuzzle = function (imageObj, puzzleNumber, puzzleSize, rows, columns){
...
game.load = function (filename, level, saveGameFlag, saveGameFlagPiople, flagPeoptleGameImg){
	game.solved = false;
	...
	if (game.flagPeoptleGameImg){
		...
		game.imageName = game.arrayBufferToUrl(arrayBufferImg, savedImgArray[0].replace('data:', ''));
	}else {
		// console.log(filename);
	...
	game.image.onload = function (){
		...
		$(game.pictureBoxImg).css({width: game.pieceInnerSize * game.puzzleQuantity.columns + 'px', height: game.pieceInnerSize * game.puzzleQuantity.rows + 'px'});
		$(game.imageHelpImg).css({width: game.pieceInnerSize * game.puzzleQuantity.columns + 'px', height: game.pieceInnerSize * game.puzzleQuantity.rows + 'px', 'background-image': 'url(' + game.imageName + ')', 'background-size': 'cover'});
		// pictureBoxHelp.css({});
...
game.touchGameEventPlay = function (){
	...
	$('.piece > svg').on(game.touchStartEvent, function (e, param){
		if ($(this).closest('.group').attr('data-flag') === 'false'){
			// console.log('start', e.target);
			e.stopPropagation();
			e.preventDefault();
			game.extactTouchPoint(e); // game.touchX = e.pageX; game.touchY = e.pageY;
			game.moving = true;
			game.movingGroupId = $(this).closest('.group').prop('id');
			game.pieceInBody = $(this).closest('.group').attr('data-flag');
			game.movingPieceId = $(this).closest('.piece').prop('id');
			game.touchStartX = game.touchX;
			game.touchStartY = game.touchY;
			game.movingGroupStartX = $(this).closest('.group').offset().left;
			game.movingGroupStartY = $(this).closest('.group').offset().top;
			...
			if ($(this).closest('.group').children().length < 2){
				game.svgShodow = $(this).closest('.group').find('.piece-svg_start');
			...
			if ($(e.target).hasClass('puzzle-border')){
				if ($(this).closest('.group').attr('data-group') === 'false'){
			...
			}else if ($(this).closest('.group').attr('data-group') === 'true'){
				...
				if (game.touchY < game.maxHeight){
					...
					group.children('.piece').each(function (){
						...
						if ( $('.group').length === 1 ) {
							...
							// music game ---------------------
							game.sounEffect('win.wav', 'win.mp3', 'win.ogg', '0.5');
							// --------------------------------
							...
							const littleWidth = (game.pieceInnerSize * game.puzzleQuantity.columns) - ((game.pieceInnerSize * game.puzzleQuantity.columns) / 100 * 30);
							const littleHeight = (game.pieceInnerSize * game.puzzleQuantity.rows) - ((game.pieceInnerSize * game.puzzleQuantity.rows) / 100 * 30);
							$(game.gameCompliteImg).css({width: littleWidth + 'px', height: littleHeight + 'px', 'background-image': 'url(' + game.imageName + ')', 'background-size': 'cover'});
...
game.checkCouple = function (source, targetId, x, y){
	...
	if (
		...
		$(game.svgShodow).closest('.group').children().each((key, elem) => {
			$(elem).find('.piece-svg_start').removeClass('piece-svg_start').removeClass('piece-svg_move').addClass('piece-svg_end');
		});
		$('.game-content__game').find('[data-group=false]').each((k, el) => {
...
game.resize = function (){
	...
	$(game.imageHelpImg).css({width: game.pieceInnerSize * game.puzzleQuantity.columns + 'px', height: game.pieceInnerSize * game.puzzleQuantity.rows + 'px'});

	const littleWidth = (game.pieceInnerSize * game.puzzleQuantity.columns) - ((game.pieceInnerSize * game.puzzleQuantity.columns) / 100 * 30);
	const littleHeight = (game.pieceInnerSize * game.puzzleQuantity.rows) - ((game.pieceInnerSize * game.puzzleQuantity.rows) / 100 * 30);
	$(game.gameCompliteImg).css({width: littleWidth + 'px', height: littleHeight + 'px'});
	...
	$(game.gameFloorPuzzleLine).css('left', (lineCssLeft / 100 * procent) + lineCssLeft);
	$(game.gameFloorPuzzleLine).prop('left', (linePropLeft / 100 * procent) + linePropLeft);
...
game.gameStartSave = function (){
	if ('indexedDB' in window){
		...
		openRequest.onupgradeneeded = function (e){  // сробатывает первый раз
			const thisDB = e.target.result;
			if (!thisDB.objectStoreNames.contains('saveGame')){ // если таблицы нет, то создать ее
