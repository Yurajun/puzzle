...
game.extactTouchPoint = function (e){
	if (typeof e.pageX === 'undefined'){
		game.touchX = e.pageY;// e.originalEvent.touches[0].pageX || e.originalEvent.changedTouches[0].pageX;
		// console.log('und');
	}else {
		game.touchX = e.pageX;
	}
	if (typeof e.pageY === 'undefined'){
		game.touchY = e.pageY;// e.originalEvent.touches[0].pageY || e.originalEvent.changedTouches[0].pageY;
	}else {
		game.touchY = e.pageY;
	}
	return true;
...
game.touchGameEventPlay = function (){
	...
	$('.piece > svg').on(game.touchStartEvent, function (e, param){
		...
		if ($(e.target).hasClass('puzzle-border')){
			...
			$(this).closest('.group').css('display', 'block');
		}
	});
