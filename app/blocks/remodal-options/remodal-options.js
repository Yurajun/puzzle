...
if (!Cookies.get('puzzleLevel')) {
	...
	$('.remodal-options__label-level')
		.map(function(key, elem) {
			if (
				$(elem)
					.children('.remodal-options__input-level')
					.val() === '2'
			) {
				return elem;
			}
		})
		.removeClass('remodal-options__label-level')
		.addClass('remodal-options__label-level2');
} else {
...
$('.remodal-options__label-level, .remodal-options__label-level2').on(
	'click',
	function() {
		$(this)
			.siblings(
				'.remodal-options__label-level, .remodal-options__label-level2'
			)
			.each((ind, elem) => {
				$(elem)
					.removeClass('remodal-options__label-level2')
					.addClass('remodal-options__label-level');
			});
...
if (typeof Cookies.get('puzzleSound') === 'undefined') {
	...
	$('.sound-input__input')
		...
		.siblings()
		.removeClass('sound-input__label')
		.addClass('sound-input__label2');
	textSpanOn();
	game.soundClick();
	game.soundPuzzle();
	game.soundLine();
	...
	if (puzzleSoundVal === 'checked') {
		...
		$('.sound-input__input')
			.prop('checked', puzzleSoundVal)
			.siblings()
			.removeClass('sound-input__label2')
			.addClass('sound-input__label');
		textSpanOff();
